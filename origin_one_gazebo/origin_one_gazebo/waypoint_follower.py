#!/usr/bin/env python3
"""
Waypoint Follower para Avular Origin One

Sigue una secuencia de waypoints. Soporta dos modos de tracción:
- skid_steer: control directo (v y ω) — implementado, sirve de referencia.
- mecanum:    movimiento omnidireccional (vx, vy) — TAREA (2 TODOs).
...
Uso:
    ...
    ros2 run origin_one_gazebo waypoint_follower --ros-args -p mode:=mecanum
"""
...
class WaypointFollower(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Waypoint Follower (modo: {self.mode})')
    ...
    def control_loop(self):
        ...
        if self.mode == 'mecanum':
            # ============================================
            # CONTROL MECANUM (omnidireccional) — TAREA
            # ============================================
            # El mecanum se mueve en cualquier dirección sin reorientarse:
            # se comandan vx (adelante) y vy (lateral) en el frame del robot.
            speed = min(self.kp_linear * distance, self.linear_speed)

            # TODO 1: pasar el ángulo hacia el goal al frame del robot.
            #   Pista: angle_in_robot = normalize_angle(angle_to_goal - self.theta)
            angle_in_robot = 0.0  # <-- reemplazar

            # TODO 2: descomponer 'speed' en vx y vy usando angle_in_robot.
            #   Pista: cos(angle_in_robot) para vx, sin(angle_in_robot) para vy.
            cmd.linear.x = 0.0  # <-- reemplazar por vx
            cmd.linear.y = 0.0  # <-- reemplazar por vy

            # rotación suave opcional para mirar hacia el objetivo
            cmd.angular.z = max(-self.angular_speed,
                                min(self.angular_speed, 0.3 * self.kp_angular * angle_error))
        else:
            # ============================================
            # CONTROL SKID STEER (v y ω simultáneos) — referencia
            # ============================================
            ...
            # apunta al goal (cos=1) -> máx; perpendicular (cos=0) -> solo gira
